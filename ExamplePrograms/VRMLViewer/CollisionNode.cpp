/***********************************************************************
CollisionNode - Node class to control collision detection in VRML
scenes.
...
CollisionNode::CollisionNode(VRMLParser& parser)
	:collide(true)
	{
	/* Check for the opening brace: */
	if(!parser.isToken("{"))
		Misc::throwStdErr("CollisionNode::CollisionNode: Missing opening brace in node definition");
	parser.getNextToken();
	...
	while(!parser.isToken("}"))
		{
		if(parser.isToken("collide"))
			{
			parser.getNextToken();
			collide=SFBool::parse(parser);
			}
		else if(parser.isToken("proxy"))
			{
			/* Parse the proxy node: */
			parser.getNextToken();
			proxy=parser.getNextNode();
			}
			...
			Misc::throwStdErr("CollisionNode::CollisionNode: unknown attribute \"%s\" in node definition",parser.getToken());
		}
